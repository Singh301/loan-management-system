...
public interface LoanRepository extends JpaRepository<Loan, Long>,
    ...
    @Query("""
        ...
        """)
    Optional<Loan> findByIdWithCustomer(
    ...
    long countByCustomerCustomerIdAndLoanStatus(
            Long customerId,
            LoanStatus loanStatus);
...
FROM Loan l
WHERE l.customer.customerId=:customerId
AND l.loanStatus='APPROVED'
    """)
    BigDecimal getApprovedLoanAmount(Long customerId);

    Loan findTopByCustomerCustomerIdOrderByApplicationDateDesc(
            Long customerId);

    @Query("""
SELECT COALESCE(AVG(l.loanAmount),0)
FROM Loan l
""")
    BigDecimal getAverageLoanAmount();

    @Query("""
SELECT COALESCE(MAX(l.loanAmount),0)
FROM Loan l
""")
    BigDecimal getHighestLoanAmount();

    @Query("""
SELECT COALESCE(MIN(l.loanAmount),0)
FROM Loan l
""")
...
YEAR(l.applicationDate),

MONTH(l.applicationDate),
...
GROUP BY YEAR(l.applicationDate),

MONTH(l.applicationDate)

ORDER BY YEAR(l.applicationDate),
...
""")
    ...
    """)
    LoanAnalyticsProjection getLoanAnalyticsByType(
